...
func getJreDir(jreOption string) string {
	...
	logger.Fatal("Can not find jre folder!")
	return ""
}
...
func (c *Classpath) ReadClass(className string) ([]byte, Entry, error) {
	...
	// 优先从bootstrap classpath搜索class文件
	if data, entry, err := c.bootstrapClasspath.readClass(className); err == nil {
		return data, entry, err
	}

	// 从extensions classpath搜索class文件
	if data, entry, err := c.extensionsClasspath.readClass(className); err == nil {
		return data, entry, err
	}

	// 从app classpath搜索class文件
	return c.appClasspath.readClass(className)
}
